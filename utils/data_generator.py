    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            if random.random()>0.5:
                with open('/private/dcase22_reassembled_test/TAU-urban-acoustic-scenes-2022-mobile-development/microphone_ir.scp',"r") as ir:
                    ir_lines = ir.readlines()
                    random_ir = random.choice(ir_lines)
                    random_ir = random_ir.strip()
                impulse, sr = librosa.load(random_ir, sr=32000)
                output = signal.fftconvolve(waveform, impulse, mode='full')
                output = output[:len(waveform)]
                y_diff = np.max(waveform) - np.min(waveform)
                output_diff = np.max(output) - np.min(output)
                waveform = output * (y_diff/output_diff)
            target = hf['target'][index_in_hdf5].astype(np.float32)
...
class DcaseDataset_device(object):
    def __init__(self):
        """This class takes the meta of an audio clip as input, and return 
        the waveform and target of the audio clip. This class is used by DataLoader. 
        """
    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            audio_name = hf['audio_name'][index_in_hdf5].decode()
            device = hf['device'][index_in_hdf5].decode()
            waveform = int16_to_float32(hf['waveform'][index_in_hdf5])
            target = hf['target'][index_in_hdf5].astype(np.float32)

        data_dict = {
            'audio_name': audio_name, 'device': device, 'waveform': waveform, 'target': target}
            
        return data_dict
    
class DcaseDataset_ir_device(object):
    def __init__(self):
        """This class takes the meta of an audio clip as input, and return 
        the waveform and target of the audio clip. This class is used by DataLoader. 
        """
...
class DcaseDataset_10s(object):
    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            n_samples_per_second = len(waveform) // 10
            t_start = np.random.randint(0, len(waveform)-n_samples_per_second)
            # crop one second audio
            waveform = waveform[t_start:t_start+n_samples_per_second]
            # time shift
            if random.random()>0.3:
                waveform = np.array(waveform)
                shift_step = int(random.random() * len(waveform))
                waveform = np.roll(waveform, shift_step)
...
class DcaseDataset_10s_ir(object):
    def __init__(self):
        """This class takes the meta of an audio clip as input, and return 
        the waveform and target of the audio clip. This class is used by DataLoader. 
        """
    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            audio_name = hf['audio_name'][index_in_hdf5].decode()
            waveform = int16_to_float32(hf['waveform'][index_in_hdf5])
            # convolutional impulse response randomly
            if random.random()>0.5:
                with open('/root/DCASE2023_data/dcase22_reassembled_test/micrphone_ir.scp',"r") as ir:
                    ir_lines = ir.readlines()
                    random_ir = random.choice(ir_lines)
                    random_ir = random_ir.strip()
                impulse, sr = librosa.load(random_ir, sr=32000)
