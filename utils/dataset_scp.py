...
from utilities import (create_folder, get_filename, create_logging, 
    float32_to_int16, pad_or_truncate, read_metadata_dcase)
import config


logging.disable(logging.DEBUG)

def pack_waveforms_to_hdf5_dcase(args):
    ...
    id_to_ix = config.id_to_ix #id_to_ix:{'airport': 0, 'shopping_mall': 1, 'metro_station': 2, 'street_pedestrian': 3, 'public_square': 4, 'street_traffic': 5, 'tram': 6, 'bus': 7, 'metro': 8, 'park': 9}
    ...
    # Read csv file
    meta_dict = read_metadata_dcase(csv_path, classes_num, id_to_ix)
    ...
    audios_num = len(meta_dict['audio_path'])
    logging.info(f'audios_num:{audios_num}')
    ...
    with h5py.File(waveforms_hdf5_path, 'w') as hf:
        ...
        hf.create_dataset('device', shape=((audios_num,)), dtype='S5')
        ...
        for n in range(audios_num):
            logging.info('number{}'.format(n))
            audio_path = os.path.join(meta_dict['audio_path'][n])
            ...
            if os.path.isfile(audio_path):
                ...
                hf['device'][n] = meta_dict['device'][n].encode()
...
if __name__ == '__main__':
    ...
    parser_pack_wavs = subparsers.add_parser('pack_waveforms_to_hdf5_dcase')
    ...
    if args.mode == 'pack_waveforms_to_hdf5_dcase':
        pack_waveforms_to_hdf5_dcase(args)
