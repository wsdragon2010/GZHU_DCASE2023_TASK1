...
def create_indexes_dcase(args):
    ...
    with h5py.File(waveforms_hdf5_path, 'r') as hr:
        with h5py.File(indexes_hdf5_path, 'w') as hw:
            ...
            hw.create_dataset('device', data=hr['device'][:], dtype='S5')
...
def combine_full_indexes_dcase(args):
    ...
    paths = [path for path in paths if (
        'train' in path and 'full_train' not in path)]
    ...
    with h5py.File(full_indexes_hdf5_path, 'w') as full_hf:
        ...
        for path in paths:
            with h5py.File(path, 'r') as part_hf:
                ...
                full_hf['device'].resize((new_n,))
                full_hf['device'][n : new_n] = part_hf['device'][:]
...
if __name__ == '__main__':
    ...
    parser_create_indexes = subparsers.add_parser('create_indexes_dcase')
    ...
    parser_combine_full_indexes = subparsers.add_parser('combine_full_indexes_dcase')
    ...
    if args.mode == 'create_indexes_dcase':
        create_indexes_dcase(args)

    elif args.mode == 'combine_full_indexes_dcase':
        combine_full_indexes_dcase(args)
