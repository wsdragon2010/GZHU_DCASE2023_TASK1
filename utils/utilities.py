...
def read_metadata_dcase(csv_path, classes_num, id_to_ix):
    ...
    Returns:
      meta_dict: {'audio_name': (audios_num,), 'device': (audios_num, ), 'target': (audios_num, classes_num)}
    """
    ...
    for n, line in enumerate(lines):
        ...
        # Target(适用于单标签数据)
        ix = id_to_ix[label_ids]
        targets[n, ix] = 1
    
    meta_dict = {'audio_name': np.array(audio_names), 'device': np.array(devices), 'target': targets, 'audio_path': np.array(audio_paths)}
    return meta_dict
